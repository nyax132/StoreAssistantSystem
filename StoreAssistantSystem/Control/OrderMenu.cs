namespace StoreAssistantSystem.Control
{
    public partial class OrderMenu : UserControl
    {
        ...
        private async void button_read_Click(object sender, EventArgs e)
        {
            ...
            var result = await TcpClientServer.SendMessage("productmaster serachjan " + textBox_jan.Text);
            if (result == "notfound")
            {
                Program.SasWindow?.StatusMessage("商品が見つかりませんでした。", Color.Black);
                return;
            }
        ...
        private void SetData(string[] datas)
        {
            try
            {
                ...
                label_sellingprice.Text = "売価" + "　" + datas[2];
                label_costprice.Text = "仕入れ値" + "　" + datas[3];
                ...
                Program.SasWindow?.StatusMessage("情報を取得しました。", Color.Black);
            }
            catch (Exception ex)
            {
